...
fn normalize_html(html: &str) -> Option<String> {
    ...
    while let Some(match_start) = output[search_start..].find(STYLE) {
        ...
        let first_char = output[start + next_non_whitespace..]
            .chars()
            .next()
            .unwrap();
        ...
        let inside_braces = &output[start + next_non_whitespace..end]
            .trim_matches('{')
            .trim_matches('}');
...
impl SplitStrategy {
    fn detect(prompt: &str) -> Option<Self> {
        ...
        let has_all_questions = QUESTIONS.iter().all(|q| as_lower.lines().any(q));
