...
fn main() {
    ...
    // Print out the first prompt that can be parsed and then the output formatted prompt
    for x in deserialized {
        let content = &x.response.choices[0].message.content;
        if let Some(parsed) = ParsedResponse::new(content) {
            if let Some(validated) = ValidatedResponse::from_parsed_response(parsed) {
                println!("{}", x.prompt);
                println!("{}", content);
                let training_example = TrainingExample::new(x.prompt, validated);
                println!("{}", training_example.output);
                return;
            }
        }
    }
